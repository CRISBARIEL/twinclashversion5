...
export const ObstacleOverlay = ({ card, isBreaking = false }: ObstacleOverlayProps) => {
  ...
  // For fire, bomb, virus - always show when obstacle is present or blocked
  const hasObstacle = card.obstacle &&
    (card.obstacle === 'fire' || card.obstacle === 'bomb' || card.obstacle === 'virus' || (card.obstacleHealth ?? 0) > 0 || (card.blockedHealth ?? 0) > 0);
