/*
  ...
  1. New Tables
    - `scores`
      ...
      - `level_id` (integer) - Level ID for challenge tracking
...
CREATE INDEX IF NOT EXISTS idx_scores_level_id ON public.scores (level_id, score DESC);

ALTER TABLE public.scores ENABLE ROW LEVEL SECURITY;

CREATE POLICY "allow_anon_insert_scores" ON public.scores
  FOR INSERT
  TO anon
  WITH CHECK (true);

CREATE POLICY "allow_anon_select_scores" ON public.scores
  FOR SELECT
  TO anon
  USING (true);